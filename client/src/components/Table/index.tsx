...
import type { TableContainerProps, TableProps, TableHeadProps, TableBodyProps } from '@chakra-ui/react';
  ...
  return (
    <TableContainer {...tableContainer}>
      <Table {...table}>
        <Thead {...tHead}>
          <Tr>
            {columns.map((item) => (<Th key={item.dataIndex as Key}>{item.title}</Th>))}
          </Tr>
        </Thead>
        <Tbody {...tBody}>
          {data.map((item) => (
